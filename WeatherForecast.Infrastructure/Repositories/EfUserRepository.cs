...
public class EfUserRepository(AppDbContext context) : GenericRepository<User>(context), IUserRepository
{
    ...
    {
        return await FirstOrDefaultAsync(u => u.Username.ToLower() == username.ToLower() && !u.IsDeleted);
    }
    ...
    {
        return await AnyAsync(u => u.Username.ToLower() == username.ToLower() && !u.IsDeleted);
    }
}
